# Cursor AI Agent Rules - Compose Multiplatform CoinJoin App
...
- **CoinJoin Coordination**: Decentralized CoinJoin pool creation using Nostr protocol
- **Privacy Features**: End-to-end encryption for all communications
- **Multiplatform Support**: Android, iOS, Desktop, and Web platforms
- **Bitcoin Operations**: Bitcoin-related operations using Bitcoin KMP library
...
- **Security-first approach** for Bitcoin and CoinJoin code
...
- Examples:
  - `feat/nostr-relay-integration` - Adding Nostr relay integration
  - `fix/coinjoin-coordination-bug` - Fixing CoinJoin coordination issue
  - `refactor/privacy-encryption` - Refactoring privacy encryption logic
  - `feat/multiplatform-navigation` - Adding multiplatform navigation
  - `fix/memory-leak-pool` - Fixing memory leak in pool management
...
3. **Commit Message Structure**
   ...
   **Scope examples**: `coinjoin`, `nostr`, `privacy`, `encryption`, `ui`, `navigation`, `bitcoin`
...
4. **Commit Message Guidelines**
   ...
   - **Body**: Detailed explanation of WHAT changed and WHY
     ...
     - **Security considerations** for Bitcoin/CoinJoin code
   - **Footer**: Reference issues, breaking changes, co-authors

5. **Example Commit**
   ```
   feat(coinjoin): implement Nostr-based CoinJoin pool coordination
   
   - Added NostrRelayManager class to handle Nostr relay connections
   - Implemented CoinJoin pool creation using Nostr events
   - Created PoolCoordinator for managing pool state
   - Added end-to-end encryption for pool communications
   - Integrated Bitcoin KMP for transaction building
   
   Security considerations:
   - All pool communications are end-to-end encrypted
   - Private keys never leave the device
   - Deterministic output ordering for CoinJoin transactions
   - Input validation for all Nostr events
   
   Technical details:
   - Using Ktor WebSocket client for Nostr relay connections
   - Integrated kotlinx.serialization for Nostr event parsing
   - Implemented proper coroutine scoping for lifecycle management
   - Added retry logic with exponential backoff for relay connections
...
2. **PR Title Format**
   ...
   ```
   Examples:
   - `[FEATURE] Add Nostr relay integration for CoinJoin pools`
   - `[FIX] Resolve memory leak in pool coordination`
   - `[REFACTOR] Restructure privacy encryption module`
   - `[SECURITY] Fix deterministic output ordering in CoinJoin`
...
3. **PR Description Template**
   ...
   - [ ] Manual testing on Web/Wasm (if applicable)
...
2. **Analysis Categories**
   ...
   **D. Security Review (CRITICAL for Bitcoin/CoinJoin)**
   - Privacy leaks in CoinJoin coordination
   - Determinism issues in cryptographic operations
   - Input validation and sanitization
   - Secure random number generation
...
3. **Review Output Format**
   ...
   **PR #**: [number] | **Author**: [username] | **Status**: [open/closed]
   ...
   ## Issues Found 🔴
   ### Critical
   - [Security issues, memory leaks, crashes, privacy leaks]
   
   ### Major
   - [Logic errors, performance problems, bad patterns, determinism issues]
   ...
   ## Security Review 🔒
   - [Security-specific findings for Bitcoin/CoinJoin code]
...
5. **Automatic Review Actions**
   ...
   **Approve** if ALL of the following are true:
   ...
   - **Security review passed** (for Bitcoin/CoinJoin code)
   
   **Request Changes** if ANY of the following are true:
   - Critical issues found (security, crashes, data loss, privacy leaks)
   ...
   - **Security issues in Bitcoin/CoinJoin code**
...
5. **Post-Merge Actions**
   ...
   **C. Cleanup**
   ...
   - Update local dev branch: `git checkout dev && git pull`
...
- Use Nostr protocol for decentralized pool creation
- Implement proper event validation for Nostr events
- Ensure deterministic output ordering for CoinJoin transactions
- Handle pool state synchronization across relays
- Implement proper error handling for relay disconnections
- Manage pool lifecycle (creation, joining, completion)

### Privacy & Encryption
- All communications must be end-to-end encrypted
- Private keys never leave the device
- Use secure random number generation (not `Random()` or `Math.random()`)
- Implement proper key derivation functions
- Validate all encrypted messages before processing
- Handle encryption failures gracefully

### Bitcoin Operations
- Use Bitcoin KMP library for Bitcoin-related operations
- Validate all Bitcoin addresses before use
- Implement proper transaction building
- Handle Bitcoin RPC errors appropriately
- Ensure deterministic transaction construction
- Validate UTXO inputs before spending

### Nostr Integration
- Use Ktor WebSocket client for Nostr relay connections
- Implement proper event filtering and subscription management
- Handle relay failures with retry logic
- Validate Nostr event signatures
- Implement proper event storage and retrieval
- Handle multiple relay connections efficiently
...
4. **Quality Assurance**
   ...
   - **Security review** for Bitcoin/CoinJoin code
...
### Example 1: Implementing CoinJoin Feature
```
Task: "Implement Nostr-based CoinJoin pool creation"

Planning:
1. Add Nostr relay WebSocket dependency
2. Create NostrRelayManager class
3. Implement CoinJoin pool creation logic
4. Add pool state management
5. Create UI for pool creation
...
### Example 2: Bug Fix
```
Task: "Fix crash when Nostr relay disconnects"
...
6. Test with relay disconnection
...
```
Task: "Review PR #45 - Add CoinJoin pool coordination"
...
5. Verify CoinJoin logic correctness
6. Check security considerations
7. Test manually with different scenarios
8. Check test coverage
9. Provide comprehensive review
```
...
- **Think cross-platform** - consider Android, iOS, Desktop, Web differences
...
- **Security-critical code** - Bitcoin, CoinJoin, cryptography require extra scrutiny
